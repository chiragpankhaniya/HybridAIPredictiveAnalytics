...
if uploaded_file is not None:
    ...
    if 'Date' not in df.columns or 'Sales' not in df.columns:
        ...
        # --- SARIMA Model (train once) ---
        sarima_forecast = np.zeros(len(y_test))
        try:
            sarima_model = auto_arima(y_train, seasonal=True, m=12, stepwise=True, suppress_warnings=True)
        ...
        # Create features for entire series then split
        X_all, y_all = create_features(y, lags=3)
        split_idx = int(len(X_all) * 0.8)
        X_train, X_test = X_all[:split_idx], X_all[split_idx:]
        y_xgb_train, y_xgb_test = y_all[:split_idx], y_all[split_idx:]
        ...
        errors = np.array([
            np.mean((y_test[-min_len:] - sarima_forecast[-min_len:])**2),
            np.mean((y_test[-min_len:] - xgb_forecast[-min_len:])**2),
            np.mean((y_test[-min_len:] - lstm_forecast[-min_len:])**2)
        ])
        weights = 1 / errors
        weights /= weights.sum()
        ensemble_forecast = (
            weights[0]*sarima_forecast[-min_len:] +
            weights[1]*xgb_forecast[-min_len:] +
            weights[2]*lstm_forecast[-min_len:]
        )
        ...
        last_data = y[-12:] if len(y) >= 12 else y
        future_preds = []
        ...
        for i in range(6):
            # SARIMA next step
            try:
                sar_pred = sarima_model.predict(n_periods=1)[0]
            except:
            ...
            if len(temp_series) > 3:
                ...
                xgb_pred = temp_series[-1]

            # LSTM next step
            if len(temp_series) >= look_back:
